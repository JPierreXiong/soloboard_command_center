  ...
  try {
    ...
    try {
      ...
      const newOrder = await db.insert(schema.order).values({
        ...
        checkoutInfo: JSON.stringify({ test: true, email: testEmail }), // 必需字段
        createdAt: new Date(),
        updatedAt: new Date(),
      }).returning();
    ...
    try {
      // 5.1 更新订单状态为 paid
      const updatedOrder = await db.update(schema.order)
        .set({
          status: 'paid',
          paidAt: new Date(),
      ...
      const newSubscription = await db.insert(schema.subscription).values({
    ...
    try {
      ...
      const subscriptions = await db.select()
        ...
        .where(eq(schema.subscription.userId, testUserId));

      // 查询订单信息
      const orders = await db.select()
        .from(schema.order)
        .where(eq(schema.order.userId, testUserId));
    ...
    try {
      // 删除订阅
      await db.delete(schema.subscription).where(eq(schema.subscription.id, testSubscriptionId));
